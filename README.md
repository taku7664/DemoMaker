...
### ▶ [예시 데모를 브라우저에서 바로 열기](https://taku7664.github.io/DemoMaker/sample/%EA%B7%B8%EB%9E%98%ED%94%84%20%EB%A1%9C%EB%94%A9%20UX%20%EA%B0%9C%EC%84%A0.html)

받지 않고 그 자리에서 눌러 볼 수 있습니다. 조작 막대에서 조건을 바꿔 가며 왼쪽과 오른쪽이 어떻게
달라지는지 견주어 보세요.
...
- **스킬 본문을 고칠 때** 는 `claude/` 와 `codex/` 의 `SKILL.md` 를 함께 고칩니다. 두 파일의 차이는
  참고 자료 폴더 이름과 부르는 방법을 적은 세 군데뿐이므로, 한쪽을 고친 뒤 `diff` 로 확인하세요.
